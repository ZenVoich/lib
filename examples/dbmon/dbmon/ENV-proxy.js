var ENV = ENV || (function() {
  ...
  function generateRow(object, keepIdentity, counter) {
    ...
    if (!object.lastSample) {
      object.lastSample = proxyObject({});
    }
    if (!object.lastSample.topFiveQueries) {
      object.lastSample.topFiveQueries = proxyObject([]);
    }
    if (keepIdentity) {
  ...
  function getData(keepIdentity) {
    ...
    if (!keepIdentity) { // reset for each tick when !keepIdentity
      ...
      for (var i = 1; i <= ENV.rows; i++) {
        data.push(proxyObject({ dbname: 'cluster' + i, query: "", formatElapsed: "", elapsedClassName: "" }));
        data.push(proxyObject({ dbname: 'cluster' + i + ' slave', query: "", formatElapsed: "", elapsedClassName: "" }));
      }
    }
    if (!data) { // first init when keepIdentity
      data = [];
      for (var i = 1; i <= ENV.rows; i++) {
        data.push(proxyObject({ dbname: 'cluster' + i }));
        data.push(proxyObject({ dbname: 'cluster' + i + ' slave' }));
      }
      oldData = data;
    }
    for (var i in data) {
